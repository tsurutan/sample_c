...
#define HTTP_TCP_PORT 40001

// プロトタイプ
void http(int sockfd);
int send_msg(int fd, char *msg);
void truncate_file(char *uri, char *buf);
...
void http(int sockfd) {
    ...
    truncate_file(uri_file, html_file);
    ...
    if (strcmp(method, "GET") == 0) {
      ...
      if ((read_fd = open(html_file, O_RDONLY, 0666)) == -1) {
      ...
      send_msg(sockfd, "\r\n");
      write(1, uri_file, strlen(uri_file));
      ...
      uri_file = uri_addr + 1;
      if(strstr(uri_file, "css")) {
        sprintf(html_file, "%s", uri_file);
      } else {
        sprintf(html_file, "%s.html", uri_file);
      }
...
void truncate_file(char *uri, char *buf) {
  char *hatena_position;
  uri = uri + 1;
  /* hatena_position = strchr(uri, '?'); */
  /* uri[hatena_position - uri] = '\0'; */
  if(strstr(uri, "css")) {
    sprintf(buf, "%s", uri);
  } else {
    sprintf(buf, "%s.html", uri);
  }
}
